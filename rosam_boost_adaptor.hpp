...
namespace boost
{
  ...
  //first we register rosam_polygon_set_t as a polygon set
    ...
    template <typename input_iterator_type>
      ...
      // /// ->
      // psd.get(polygon_set);
      // polygon_set.assign(input_begin, input_end);

          std::cout << "filling rosam_polygon_set with this polygon set ==" << psd << std::endl ;
          typedef polygon_set_data<double>::iterator_type iterator_type;
          for (iterator_type itr = psd.begin(); itr != psd.end(); ++itr)
          {
            // rosam_polygon_set_t is a vector of
            // gtsam::Pose2 vectors, so we need to
            // build first each gtsam::Pose2 vector
            // and then push it back into the polygon_set

            gtsam::Pose2 _pose2(((*itr).first.first).get(HORIZONTAL),
                                ((*itr).first.second).get(VERTICAL),
                                0.0);

            rosam_polygon_t _vector_pose2;
            _vector_pose2.push_back(_pose2);
            polygon_set.push_back(_vector_pose2);
            }
          }
      };
  }
